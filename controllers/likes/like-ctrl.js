  ...
  getLike: async (req, res) => {
    try {
      ...
      if (likeUser) {
        ...
        logger.info(
          `GET /likes/:postId 404 "postId가 ${postId}의 글에 좋아요를 한 사람이 없습니다." `
        );
  ...
  createLike: async (req, res) => {
    try {
      ...
      if (!likeUser) {
        ...
        logger.info(
          `POST /likes/:postId 404 "postId가 ${postId}의 글에 ${userId}님은 이미 좋아요를 했습니다. 한번만 가능합니다." `
        );
        res.status(404).send({ msg: "좋아요는 한번만 할 수 있습니다" });
      }
    } catch (err) {
  ...
  removeLike: async (req, res) => {
    try {
      ...
      if (likeUser) {
        ...
        logger.info(
          `DELETE /likes/:postId 404 "postId가 ${postId}의 글에 ${userId}님이 좋아요를 한적이 없습니다." `
        );
        res.status(404).send({ msg: "좋아요를 한 상태에만 가능한 기능입니다" });
      }
    } catch (err) {
